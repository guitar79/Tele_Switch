...
% Last Modified by GUIDE v2.5 27-May-2016 12:06:26
...
try	
	...
	fullSplashImageName = fullfile(handles.programFolder, '/Splash Images/MakeMovie.png');
...
try
	...
	if ~isempty(handles.imageFolder)
		if exist(folder,'dir') == false
			...
			% Make it something, so that we can continue.
...
try
	...
	ListOfImageNames = get(handles.lstImageList, 'string');
	selectedFiles = get(handles.lstImageList, 'Value');
	numberOfFrames = length(selectedFiles);
	% If none are selected, select them all
	if numberOfFrames == 0
		numberOfFrames = length(ListOfImageNames);
		selectedFiles = 1 : numberOfFrames;
	end
	...
	for frame = 1 : numberOfFrames
		index = selectedFiles(frame); % Get the actual index in the listbox of whatever they selected.
		% Construct an output image file name.
		sourceBaseFileName = ListOfImageNames{index};
		...
		% Put the date into the lower left in yellow font, as long as it's not the first frame.
		if frame > 1
% 			fileInfo = imfinfo(sourceFrameFullFileName);
% 			theDate = fileInfo.FileModDate(1:12)
% 			d = dir(sourceFrameFullFileName);
% 			theDate = d.date(1:12)
			% Get date from the first 8 characters of the filename: yyyymmdd
			theYear = sourceBaseFileName(1:4);
			theMonth = sourceBaseFileName(5:6);
			theDay = sourceBaseFileName(7:8);
			theDate = sprintf('%s - %s - %s', theMonth, theDay, theYear);
			text(10, 15, theDate, 'Color', 'y', 'FontSize', 13, 'FontWeight', 'bold');
		end
		drawnow;
	...
	% Let user know we're done, and what the movie filename is.
	message = sprintf('\nYour %d frame movie is in file:\n%s', numberOfProcessedFiles, fullFileName);
	set(handles.txtInfo, 'String', message);
	drawnow;
	message = sprintf('\nYour %d frame movie is in file:\n%s\n\nClick OK to play it', numberOfProcessedFiles, fullFileName);
	msgboxh(message);

	% Play the movie in the default program (like Windows Media Player).
	% I think winopen() only works for the Windows operating system.
	if strfind(computer, 'PCWIN')
		% It's a windows computer.
		winopen(fullFileName);
	else
		% It's a not a Windows computer.
		disp('iMac');
	end
...
% 	movie(constructedMovie);
	...
	errorMessage = sprintf('Error in program %s, function %s(), at line %d.\n\nError Message:\n%s', ...
		mfilename, ME.stack(1).name, ME.stack(1).line, ME.message);
	if ~isempty(strfind(errorMessage, 'Permission Denied'))
		% They probably still have the movie open in the Video Player (Windows Media Player).
		errorMessage = sprintf('%s\n\nIf you have the movie open in the Video Player program,\nyou must close it before a new one can be created.', errorMessage);
	end
	WarnUser(errorMessage);
end
...
try
	...
	case 'radSortAlpha'
		...
		txtInfo = sprintf('Files will be sorted alphanumerically.');
		ListOfImageNames = get(handles.lstImageList,'string');
		ListOfImageNames = sort(ListOfImageNames);
		set(handles.lstImageList, 'String', ListOfImageNames);
	case 'radSortTime'
		% Code for when radiobutton2 is selected.
		txtInfo = sprintf('Files will be sorted by file date/time.');
		WarnUser('Not implemented yet');
	otherwise
...
try
	...
	framesPerSecond = scrollbarValue;
	caption = sprintf('Frame Rate = %.2f frames per second', framesPerSecond);
	set(handles.txtFrameRate, 'string', caption);
	% Find out how many frames are selected.
	selectedListboxItem = get(handles.lstImageList, 'value');
	if isempty(selectedListboxItem)
		% None are selected.  Find out how many are in the listbox.
		ListOfImageNames = get(handles.lstImageList, 'string');
		numFrames = length(ListOfImageNames);
	else
		numFrames = length(selectedListboxItem);
	end
	txtInfo = sprintf('Frame Rate = %.2f frames per second.\nTotal movie length = %.1f seconds.', framesPerSecond, numFrames / framesPerSecond);
	handles.txtInfo.String = txtInfo;
catch ME
	errorMessage = sprintf('Error in program %s, function %s(), at line %d.\n\nError Message:\n%s', ...
		mfilename, ME.stack(1).name, ME.stack(1).line, ME.message);
	WarnUser(errorMessage);
...
try
	...
	defaultFileName = fullfile(startingFolder, 'Screenshot.png');
...
% --- Executes on button press in radSortTime.
function radSortTime_Callback(hObject, eventdata, handles)
% hObject    handle to radSortTime (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% Hint: get(hObject,'Value') returns toggle state of radSortTime


% --- Executes on button press in radSortAlpha.
function radSortAlpha_Callback(hObject, eventdata, handles)
% hObject    handle to radSortAlpha (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% Hint: get(hObject,'Value') returns toggle state of radSortAlpha
...
try
	...
	guiSettings.radSortAlpha = get(handles.radSortAlpha, 'Value');
	guiSettings.radSortTime = get(handles.radSortTime, 'Value');
...
try
	...
	if exist(matFullFileName, 'file')
		...
		radSortAlpha = initialValues.guiSettings.radSortAlpha;
		radSortTime = initialValues.guiSettings.radSortTime;
		...
		set(handles.radSortAlpha, 'Value', radSortAlpha);
		set(handles.radSortTime, 'Value', radSortTime);
	else
